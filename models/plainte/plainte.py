...
class Plainte(models.Model):
    ...
    reference = fields.Char(string="Plainte No", readonly=True,
                            required=True, copy=False, default=_init_lang)
    date_appel = fields.Datetime(
        string="Date d'appel", required=True, default=lambda self: fields.datetime.now())
    ...
    region_id = fields.Many2one('mgp.loc_region', store=False,
                                required=True, string="Région", default=_get_default_region)
    ...
    # Localisation : Non stockée dans la base mais juste pour faciliter la recherche de commune (selectoin de dependance)
    district_id = fields.Many2one('mgp.loc_district', store=False,
                                  required=True, string="District", default=_get_default_district)

    # Localisation à enregistrer (Obligatoire)
    commune_id = fields.Many2one(
        'mgp.loc_commune', ondelete='cascade', string="Commune", required=True)

    # Localisation à enregistrer (Facultative)
    fokontany_id = fields.Many2one(
        'mgp.loc_fokontany', ondelete='cascade', string="Fokontany")
    ...
    situation = fields.Selection([
        ('joignable', 'joignable'),
        # important 2021/04/29 => statu = state_invalid
    ...
    statut = fields.Selection([
        ...
        # EN évaluation chez PREA
        ('state_eval_response_prea', 'A évaluer par PREA'),
        # Donner la réponse au citoyen par BPO
        ('state_send_response_bpo', 'A traiter par BPO'),
        # Le traitment du ticket est terminé
        ('state_done_bpo', 'Ticket traité'),
        # Ticket invalide par le PREA (non exploitable)
    ...
    def _get_categorie_wrap(self):
        ...
        for rec in self:
            rec.categorie_wrap = ' '.join(
                [s for s in rec.categorie_id.name.split(' ')[0:5]]) + ' ...'

    statut_display = fields.Text(
        string="Statut du ticket", compute='get_statut')

    def get_statut(self):
        """ Champs calculé qui renvoie le satut du ticket """
        self.statut_display = dict(
            self._fields['statut'].selection).get(self.statut)

    statut_response_display = fields.Text(
        string="Réponse envoyée", compute='check_response')

    def check_response(self):
        """ Renvoie le statut de la réponse"""
        self.statut_response_display = 'Oui' if self.response_complete else 'Non'

    situation_display = fields.Text(
        string="Situation", compute='get_situation')

    def get_situation(self):
        self.situation_display = dict(
            self._fields['situation'].selection).get(self.situation)

    result_display = fields.Text(string="Résultat", compute='get_result')

    def get_result(self):
        self.result_display = dict(
            self._fields['resultat'].selection).get(self.resultat)
    ...
    def _get_log_ids(self):
        for rec in self:
            rec.log_ids = self.env['mgp.plainte_log'].search(
                [('plainte_id', '=', rec.id)])

    # NAME; Dernier group qui detient le ticket  (Le group receveur)
    actual_group_name = fields.Char(
        string="Responsable actuel", compute="get_actual_group_name")

    def get_actual_group_name(self):
        for rec in self:
            log = self.env['mgp.plainte_log'].search(
                [('plainte_id', '=', rec.id)], order='create_date desc', limit=1)
            if log is not None:
                ...
                if rec.statut == 'state_traitement_pmo' and self.env.user.has_group('mgp.mgp_gouvernance_prea'):
                    # Renvoyer le nom de l'utilisateur du PMO
                    rec.actual_group_name = rec.user_pmo_name
    ...
    jours_traitement = fields.Integer(
        string="Jour(s) de traitement", compute="get_jours_traitement")

    def get_jours_traitement(self):
        """ Renvoie le nombre de jour de traitement du ticket """
        for rec in self:
            import datetime
            d1 = datetime.date(rec.date_appel.year,
                               rec.date_appel.month, rec.date_appel.day)
    ...
    def _get_localisation(self):
        ...
        for rec in self:
            rec.zone = '{} / {} / {}'.format(rec.region_id.name,
                                             rec.district_id.name, rec.commune_id.name)
            if rec.fokontany_id:
    ...
    check_write_resultat_situation = fields.Boolean(
        compute='_check_write_resultat_situation')
    ...
    reprocessed_display = fields.Text(
        string="Retraitement", compute='get_reprocessed')
        ...
        for rec in self:
            if len(rec.ennonce) < 10:
                ...
                raise ValidationError(
                    _("L'ennoncé de la plainte doît contenir au moins 10 caractères"))
    ...
    def create(self, vals):
        ...
        rec = super(Plainte, self.with_context(
        ...
        status_sms_code = self.send_sms(rec, log)
    ...
    def unlink(self):
        ...
        if self.statut == 'state' and resp_group_id == current_group_id:
            ...
            raise ValidationError(
                _("Le ticket n° {} ne peut plus être supprimé.".format(self.reference)))
            # ???
            # return {
            #     'type': 'ir.actions.client',
            #     'tag': 'display_notification',
    ...
    def action_send_to_prea(self):
        ...
        if self.env.user.has_group('mgp.mgp_gouvernance_operateur'):
            for rec in self:
                ...
                log = self._do_log(
                    plainte_id=rec.id,
                    group_sender_id=self.env.ref(
                        'mgp.mgp_gouvernance_operateur').id,
                    group_receiver_id=self.env.ref(
                        'mgp.mgp_gouvernance_prea').id,
                    action="Envoi du ticket aux admin PREA",
                    statut="state_validate_prea",
                    notif_sender="Ticket n° {} envoyé au PREA".format(
                        rec.reference),
        ...
        if self.env.user.has_group('mgp.mgp_gouvernance_operateur'):
            for rec in self:
                ...
                log = self._do_log(
                    ...
                    notif_sender="Ticket n° {}, citoyen joignable".format(
                        rec.reference),
        ...
        if self.env.user.has_group('mgp.mgp_gouvernance_operateur'):
            for rec in self:
                ...
                log = self._do_log(
                    ...
                    notif_sender="Ticket n° {}, citoyen injoignable".format(
                        rec.reference),
    ...
    def call_back_customer(self):
        ...
        if self.env.user.has_group('mgp.mgp_gouvernance_operateur'):
            for rec in self:
                ...
                ticket_number = rec.reference
                phone = rec.tel

                # 2 - call customer
                call_response = self.use_vicidial_call(
                    phone=phone, bpo=login, ticket_number=ticket_number)
                call_status = "state_call_back_success" if call_response else "state_call_back_error"
                call_status_message = "joinbable" if call_response else "injoinbable ou erreur"


                # 3 - Log : customer call state
                self._do_log(
                    plainte_id=rec.id,
                    group_sender_id=self.env.ref(
                        'mgp.mgp_gouvernance_operateur').id,
                    group_receiver_id=self.env.ref(
                        'mgp.mgp_gouvernance_prea').id,
                    action="Appeler le citoyen",
                    statut=call_status,
                    notif_sender="Ticket n° {}, numéro citoyen {}, appelant {}".format(
                        rec.reference, phone, login),
                    notif_receiver="Appel ticket n° {}, citoyen {}".format(rec.reference, call_status_message))
        else:
            return {
                'type': 'ir.actions.client',
                'tag': 'display_notification',
                'params': {
                    'title': _("Appeler le citoyen"),
                    'message': _("Seul l'opérateur BPO peut faire cette action."),
                    'type': 'danger',
                    'sticky': False,
                },
        ...
        if self.env.user.has_group('mgp.mgp_gouvernance_operateur'):
            for rec in self:
                ...
                log = self._do_log(
                    ...
                    notif_sender="Ticket n° {}, citoyen satisfait".format(
                        rec.reference),
        ...
        if self.env.user.has_group('mgp.mgp_gouvernance_operateur'):
            for rec in self:
                ...
                log = self._do_log(
                    ...
                    notif_sender="Ticket n° {} renvoyé par PREA".format(
                        rec.reference),
            ...
            for rec in self:
                ...
                log = self._do_log(
                    ...
                    group_sender_id=self.env.ref(
                        'mgp.mgp_gouvernance_prea').id,
                    group_receiver_id=self.env.ref(
                        'mgp.mgp_gouvernance_pmo').id,
                    action="Envoi du ticket au PMO ({})".format(
                        rec.user_pmo_id.name),
                    statut="state_traitement_pmo",
                    notif_sender="Ticket n° {} envoyé au PMO".format(
                        rec.reference),
        ...
        if self.env.user.has_group('mgp.mgp_gouvernance_pmo'):
            for rec in self:
                ...
                log = self._do_log(
                    ...
                    group_receiver_id=self.env.ref(
                        'mgp.mgp_gouvernance_prea').id,  # Traitement fini
                    action="Envoi de réponse au PREA",
                    statut="state_eval_response_prea",
                    notif_sender="Ticket n° {}, réponse envoyée au PREA".format(
                        rec.reference),
    ...
    def action_return_ticket_to_prea(self):
        ...
        """
        for rec in self:
            rep = self.env['mgp.plainte_reponse'].search(
                [('plainte_id', '=', rec.id)], limit=1)
            if rep.id:
                return {
                    'type': 'ir.actions.client',
                    'tag': 'display_notification',
                ...
                log = self._do_log(
                    ...
                    statut="state_validate_prea",
                    notif_sender="Ticket n° {} retourné au PREA".format(
                        rec.reference),
                    notif_receiver="Ticket n° {} en attente de validation".format(rec.reference))
                ...
                action = self.env.ref(
                    'mgp.plainte_pmo_action_window').sudo().read()[0]
                return action
        ...
        for rec in self:
            if not rec.user_pmo_id.id:
                ...
                log = self._do_log(
                    ...
                    statut="state",
                    notif_sender="Ticket n° {} renvoyé au BPO".format(
                        rec.reference),
                    notif_receiver="Ticket n° {} reçu du PREA".format(rec.reference))
        ...
        for rec in self:
            ...
            if rep.id and rec.response_complete and rec.user_pmo_id.id:
                ...
                log = self._do_log(
                    ...
                    group_receiver_id=self.env.ref(
                        'mgp.mgp_gouvernance_pmo').id,
                    action="Renvoi du ticket au PMO ({}) (cause: mauvaise réponse)".format(
                        rec.user_pmo_id.name),
                    statut="state_traitement_pmo",
                    notif_sender="Ticket n° {} renvoyé au PMO".format(
                        rec.reference),
                    notif_receiver="Ticket n° {} reçu du PREA".format(rec.reference))
    ...
    def _get_groups_from_user(self, user_id):
        ...
        groups_ids = []
        self.env.cr.execute(
            "SELECT gid FROM res_groups_users_rel WHERE uid="+str(user_id))
    ...
    def assigner_tiket_user_pmo(self):
        if not self.user_pmo_id:
            title = 'Assigner le ticket n°{} à un utilisateur du PMO'.format(
                self.reference)
    ...
    user_pmo_name = fields.Char(
        compute='_get_user_pmo_name', default="", store=False)
    ...
    def _get_body_prea(self):
        ...
        for o in res:
            if o[0] in ('state_validate_prea',):
                message += "- {} ticket(s) en attente de validation <br>".format(
                    o[1])
            if o[0] in ('state_eval_response_prea',):
                message += "- {} ticket(s) avec réponse complète à envoyer au BPO<br>".format(
                    o[1])
            if o[0] in ('state_done_bpo',):
                message += "- {} ticket(s) en attente de fermeture<br>".format(
                    o[1])
            total += o[1]
    ...
    def send_email_pmo_job(self):
        ...
        for email in self._get_emails_to("mgp.mgp_gouvernance_pmo"):
            sender_id = self.env['res.users'].search(
    ...
    def send_sms(self, rec, log):
        ...
        if rec:
            ...
            # 2- Send sms
            if sms:
                # Find {reference} in the record and replace it
                sms_tmps = sms.message.replace("{reference}", rec.reference)
                status_code = self.send_sms_to_customer(customer_number=rec.tel, message=sms_tmps)
    ...
    def send_sms_to_customer(self, customer_number="", message=""):
        """
        ...
        """
        status_code = 400
        urlAuth = "https://api.orange.com/oauth/v3/token"
        payload='grant_type=client_credentials'

        headers = {
            'Content-Type': 'application/x-www-form-urlencoded',
            'Authorization': 'Basic N0V3SEM1dUJ5WWJmRzZsVE5VYjZsT282UUE4TTF2UWE6TFlaVURiT20zR0FkaXFBaw=='
        }

        response_auth = requests.request("POST", urlAuth, headers=headers, data=payload)
        authorization = json.loads(response_auth.text)
        url = "https://api.orange.com/smsmessaging/v1/outbound/tel%3A%2B261/requests"
        
        try:
            payload = json.dumps({
                "outboundSMSMessageRequest": {
                    "senderAddress": "tel:+261",
                    "address": f'tel:+261{customer_number[1:]}',
            ...
            print("Sending SMS status: {} ,Sent to: {}".format(response.status_code, customer_number))
    ...
    def use_vicidial_call(self, phone="", bpo="", ticket_number=""):
        try:
            url = 'http://3.7.133.71/r1/ugd-poc/COM/HAIRUN/MGP/call_trigger'

            headers = {
                'Content-Type': 'application/json',
                'X-Road-Client': 'ugd-poc/GOV/MANAGEMENT/EKORAGNA'
            }


            data = {"vcdl_phone_number": phone[1:], "vcdl_agent": bpo, "mgp_ticket": ticket_number}

            response = requests.request(
                "POST", url, headers=headers, data=json.dumps(data))

            status_code = (response.status_code == 200) if True else False
    ...
    def open_popup_list_search_tickets(self):
        return {
            ...
            'type': 'ir.actions.act_window',
            # 'plainte_bpo_list_search_action_window',
            'view_id': self.env.ref('mgp.plainte_bpo_list_search_view').id,
            'view_mode': 'tree',
            'res_model': 'mgp.plainte',
            'target': 'new'
        }
